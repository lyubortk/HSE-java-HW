...
public class HashtableCopy {
    ...
    private void checkBucketsNumber() {
        if (size * 2 > bucketsNumber) {
            var newHashtable = new HashtableCopy(bucketsNumber * 2);
    ...
    private void copyFrom(HashtableCopy table) {
        size = table.size;
        bucketsNumber = table.bucketsNumber;
        bucketArray = table.bucketArray;
    }

    /** Adds (by calling method {@link HashtableCopy#put}) every pair of (key, value) stored in the
     * hash table to another table.
     * @param table another hash table to copy all stored data to */
    private void copyContentTo(HashtableCopy table) {
